...
For each training example a loss function is minimized, so the error on the labled training data gets smaller. 
...
We've also tried Tanh and Maxout as activation function, with comparable results.

Since we are training our model rather small dataset the classes were balanced during training using the balance_class parameter of the h2o-package. Internal validation is done using 10-fold cross-validation with stratified assignment. 
