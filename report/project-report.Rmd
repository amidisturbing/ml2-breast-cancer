...
### Model overview
Artificial Neural Networks are forecasting methods based on simple mathematical models of the brain. They allow complex nonlinear relationships between the response variable and its predictors. (src: https://otexts.com/fpp2/nnetar.html) 

<here IMG ANN>
FIGURE 1. Sample artificial neural network architecture (not all weights are shown). (src: https://www.sciencedirect.com/topics/computer-science/artificial-neural-network)

Connections between neurons are weighted to represent the connection strength. It's the artificial pendant of the synapses in the brain.
...
Performing a grid-search we've chosen the model with the highest auc-value which isn't 1. This decision is based on preventing overfitting. The False-Positives were examined manually to get a grip on the real-world-value of our model.  What we are trying to prevent is our model telling that a person doesn't have breast-cancer, when in fact he or she does.  
...
We compare the logistic regression and the neural networks wrt accuracy and sensitivity and specificity on the test set. \cite{Abril07}
